...
def get_kernel(a=0.6):
    ...
    # kernel = np.array([[0.00391, 0.01562, 0.02344, 0.01562, 0.00391],
    #                    [0.01562, 0.06250, 0.09375, 0.06250, 0.01562],
    #                    [0.02344, 0.09375, 0.14062, 0.09375, 0.02344],
    #                    [0.01562, 0.06250, 0.09375, 0.06250, 0.01562],
    #                    [0.00391, 0.01562, 0.02344, 0.01562, 0.00391]])

    # return cv2.getGaussianKernel(ksize=5, sigma=0.2)

    return np.array([[0.00391, 0.01562, 0.02344, 0.01562, 0.00391],
                       [0.01562, 0.06250, 0.09375, 0.06250, 0.01562],
                       [0.02344, 0.09375, 0.14062, 0.09375, 0.02344],
                       [0.01562, 0.06250, 0.09375, 0.06250, 0.01562],
                       [0.00391, 0.01562, 0.02344, 0.01562, 0.00391]])
...
def gen_file_list(in_dir, out_f_path, ext='.jpg'):
    """
    """
    if not os.path.isdir(in_dir):
        print('[Err]: invalid input dir.')
        return

    img_name_list = [x for x in os.listdir(in_dir) if x.endswith(ext)]
    with open(out_f_path, 'w', encoding='utf-8') as f:
        for img_name in img_name_list:
            f.write(img_name + '\n')
    print('{:s} written.'.format(out_f_path))


if __name__ == '__main__':
    gen_file_list(in_dir='./image_set/test17',
                  out_f_path='./test17_img_name_list.txt',
                  ext='.png')
