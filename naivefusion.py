...
class WeightsMap(object):
    ...
    def __init__(self, dir_path, img_name_list):
        """
        ...
        for img_name in img_name_list:
            self.images.append(image.Image(dir_path, img_name))
    ...
    def result_exposure(self, w_c=1, w_s=1, w_e=1):
        ...
        for channel in range(3):
            for index in range(self.num_images):
                self.result_image[:, :, channel] += self.weights[index] * self.images[index].array[:, :, channel]
